...
def eval_depth(loader, folder):
    ...
    for i, sample in enumerate(loader):
        ...
        ## valid_mask = (label != 0)
        valid_mask1 = (label != 0)
        valid_mask2 = (label != 255)
        valid_mask3 = (label < 65535 * 0.01 * 0.01)
        valid_mask = np.multiply(valid_mask1, valid_mask2)
        valid_mask = np.multiply(valid_mask, valid_mask3)
...
class DepthMeter(object):
    ...
    def update(self, pred, gt):
        ...
        ## mask = (gt != 255).bool()
        mask1 = (gt != 255).bool()
        mask2 = (gt < 65535 * 0.01 * 0.01).bool()
        mask3 = (gt != 0).bool()
        mask = mask1.float() * mask2.float() * mask3.float()
        mask = mask.bool()
